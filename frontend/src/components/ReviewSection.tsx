    ...
    <div className="space-y-6">
        ...
        <div className="bg-primary/5 p-6 rounded-2xl border border-primary/20">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-black text-sm text-primary uppercase">
              {lang === 'ar' ? 'ملخص الذكاء الاصطناعي للآراء' : 'AI Review Summary'}
            </h3>
          </div>
        ...
        <form onSubmit={handleReviewSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex gap-4">
            <input
              ...
              className="flex-1 bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-primary transition-colors"
            />
            <Button type="submit" disabled={reviewLoading} className="px-6 rounded-xl font-black">
      ...
      <div className="space-y-4">
        {reviews.length === 0 ? (
          ...
          reviews.map((review) => (
            <div key={review.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-3">
